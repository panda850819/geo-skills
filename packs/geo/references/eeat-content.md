...
> E-E-A-T 4x25pts detailed signal tables, content quality metrics, AI content signals. Used by geo-audit (Agent 4: Content).

## Overview

AI search platforms do not just find content — they evaluate whether content deserves to be cited. The primary framework is **E-E-A-T** (Experience, Expertise, Authoritativeness, Trustworthiness), which per Google's December 2025 Quality Rater Guidelines update now applies to **ALL competitive queries**, not just YMYL topics.
...
- Hedging language suggesting lack of direct knowledge ("reportedly", "supposedly", "some say")
...
Floors, not targets. More words does not mean better content. The benchmark is the minimum length to adequately cover a topic for AI citability.
...
- NOT a direct ranking factor but affects citability — AI platforms prefer clear and unambiguous content
- Overly academic (score < 30) reduces citability for general queries
- Overly simple (score > 80) may lack depth for expertise signals

**Estimation without a tool:**
...
- **No skipped levels** — do not go from H1 to H3 without H2
- **Descriptive headings** — "How to Optimize for AI Search" not "Section 2"
- **Question-based headings** where appropriate — map directly to AI queries

### Internal Linking

- Every content page should link to 3-5 related pages on the same site
- Descriptive anchor text (not "click here")
- Topic cluster structure: pillar page linked to/from all related subtopic pages
- Orphan pages (no internal links) are rarely cited by AI
...
- Check for visible `datePublished` and `dateModified` in both content and structured data
...
Flag content as evergreen if:
- Covers fundamental concepts that do not change (physics, basic math, legal definitions)
- Clearly labeled as a reference/guide for lasting concepts
- Does not contain time-dependent claims ("the latest", "currently", "in 2024")
...
Topical authority measures whether a site comprehensively covers a topic rather than touching on it superficially. AI platforms prefer citing sites that are recognized authorities.

### How to Assess

1. **Content breadth**: Multiple pages covering different aspects of core topic?
2. **Content depth**: Individual pages go deep into subtopics?
3. **Topic clustering**: Pages organized into logical groups with internal linking?
4. **Content gaps**: Obvious subtopics the site should cover but does not?
5. **Competitor comparison**: Competitors cover subtopics this site misses?
